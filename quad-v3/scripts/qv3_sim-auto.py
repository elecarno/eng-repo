...
if __name__ == "__main__":
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        # ─── ADDED: AUTOMATIC CAMERA TRACKING ─────────────────────────────────
        # Body ID 0 is the world, so Body ID 1 is almost always the robot's main root body
        viewer.cam.type = mujoco.mjtCamera.mjCAMERA_TRACKING
        viewer.cam.trackbodyid = 1  
        viewer.cam.distance = 2.0   # Adjust this to zoom closer or further out
        viewer.cam.elevation = -20  # Look slightly down on the robot
        # ──────────────────────────────────────────────────────────────────────
        ...
        while viewer.is_running():
            ...
            state_change = random.random()
            change_mod = 30
            if round(t*10) % change_mod == 0:
                if state_change <= 0.6:
                    state = "walk"
                    change_mod = 30
                elif state_change > 0.6 and state_change <= 0.7:
                    state = "right"
                    change_mod = 10
                elif state_change > 0.7 and state_change <= 0.8:
                    state = "left"
                    change_mod = 10
                elif state_change > 0.8 and state_change <= 0.9:
                    state = "back"
                    change_mod = 5
                else:
                    state = "dance"
