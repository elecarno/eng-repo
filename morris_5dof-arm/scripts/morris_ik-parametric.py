...
if __name__ == "__main__":
    ...
    try:
        ...
        print("STARTING PARAMETRIC TRAJECTORY...")
        
        # Trajectory Configuration
        start_time = time.time()
        duration = 10.0  # Run the parametric loop for 10 seconds
        dt = 0.05        # Time step between updates (20 Hz refresh rate)
        
        # Circle parameters (Example)
        center_x, center_y, center_z = 0, -0.12, 0.15
        radius = 0.05
        frequency = 0.5  # Revolve once every 2 seconds (0.5 Hz)
        ...
        while True:
            ...
            # -----------------------------------------------------------------
            # PARAMETRIC EQUATIONS (Example: Circle in XY plane)
            # -----------------------------------------------------------------
            # x(t) = cx + r * cos(2 * pi * f * t)
            # y(t) = cy + r * sin(2 * pi * f * t)
            # z(t) = cz
            x = center_x + radius * np.cos(2 * np.pi * frequency * t)
            y = center_y
            z = center_z + radius * np.sin(2 * np.pi * frequency * t)
            
            target_3d = np.array([x, y, z])
            # -----------------------------------------------------------------
            ...
            # Map joint angles to pulse widths
            target_positions = [
            ...
            # Send current target position data to the hardware
