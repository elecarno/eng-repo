...
\end{align*}
where $h$ is the vertical offset of $J_{1}$ from the origin of $\{\mathrm{s}\}$. Link 2 posssesses an L-shaped structure, as such an "offset angle" $\theta_{0}$ is established, and the length of the effective link $L_{2}$ is calculated from its components,
\begin{align*}
...
The arm has a limited range and as such if a target point $\vec{p}$ is outside of this range the $\theta_{n}$ functions may fail to produce values. For a point to be in the workspace of the robot the expression ///
